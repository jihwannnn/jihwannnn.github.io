...
* 생성모델, 주어진 학습 데이터를 학습하여 학습 데이터의 분포를 따르는 유사한 데이터를 생성하는 모델. 학습 데이터의 분포를 학습하는 것이 주요한 object가 된다고 할 수 있다.

**Latent Variable Model**

* Latent Variable: 잠재 변수, 모델 밖에서 관측이나 측정되지 않는 내부에서의 특정 상태.

* 잠재 변수 모델은 관찰된 데이터로부터 숨겨진 구조를 밝혀내기 위한 ***확률적 모델***의 한 유형. 따라서 입력 데이터로부터 ***단순한 분포***를 따르는 latent variable(z)를 얻어내고, 이를 내가 원하는 ***복잡한 분포로 매핑***하여 변환하는 역량을 학습하고자 한다.

**VAE (variational autoencoder)**

![Diffusion image 1](/assets/image/14_Diffusion/Diffusion_1.png)

* Encoder, Decoder와 latent space를 다 포함하는 모델 자체를 지칭한다.

**GAN**

![Diffusion image 2](/assets/image/14_Diffusion/Diffusion_2.png)

* latent variable을 특정한 패턴의 분포로 mapping 한다. encoder & decoder이 아닌 generator & discriminator 구조로 학습.

**Flow-Based Model**

![Diffusion image 3](/assets/image/14_Diffusion/Diffusion_3.png)

* 가역적인 함수로 구성

**Diffusion (확산)**

* 정의: 특정 물질이 일률적인 농도를 갖도록 퍼지는 물리적 현상

* 특정한 데이터의 패턴이 서서히 반복적인 과정을 거쳐 와해되는 과정을 ***Diffusion Process***라 명명, 대표적인 비지도 학습 (unsupervised learning) 방법론. 신기한 것은 forward와 reverse모두 gaussian으로 구성되어 있다는 것.

* ***Markov Chain***: t+1 시점의 상태의 확률은 오직 t 시점의 상태에 의존한다. $P[s_{t+1}\mid s_t] = P[s_{t+1}\mid s_1,...,s_t]$

***모델 간의 차이점***

![Diffusion image 4](/assets/image/14_Diffusion/Diffusion_4.png)

* nosing & denoising: 패턴에 노이즈를 추가하여 latent variable을 생성한 후, 이를 다시 복원하는 ***조건부 PDF를 학습***

![Diffusion image 5](/assets/image/14_Diffusion/Diffusion_5.png)

* ***Forward Process (Diffusion Process)***: gaussian noise를 점진적으로 주입
    * $q(X_t \mid X_{t-1}) := N(X_t;\mu_{X_{t-1}},\Sigma_{X_{t-1}}) := N(X_t;\sqrt{1-\beta_t}X_{t-1}, \beta_t \cdot I)$

    * $\beta_t$: t시점의 noise scheduling parameter, gaussian noise를 ***점진적으로*** 주입하기 위함

        1. Linear schedule
        2. Quad schedule
        3. Sigmoid schedule

        ![Diffusion image 6](/assets/image/14_Diffusion/Diffusion_6.png)
    
    * $X_t = \sqrt{1-\beta_t}X_{t-1} + \sqrt{\beta_t}\epsilon_{t-1}, \\ \epsilon \sim N(0,1)$ > Reparameterization Trick이라고 한다

    * $q(X_{1:T} \mid \mathbf{X_0}) := \Pi^T_{t=1}q(X_t \mid X_{t-1})$ > 결론적으로 Diffusion Process는 Conditional Gaussian의 joint-distribution으로서, ${X_0}$ 의 조건부로 latent variable($X_{1:T}$)를 생성하는 과정이라고 할 수 있다. 마지막 $X_T = Z_T$ 는 pure isotropic gaussian.

* ***Reverse Process***: Denosing 과정을 ***학습***

![Diffusion image 7](/assets/image/14_Diffusion/Diffusion_7.png)

* Loss Function

![Diffusion image 8](/assets/image/14_Diffusion/Diffusion_8.png)
